...
Usage:
    ...
    Example equivalent commands to run (single node, 2 gpu; top is for bio-lambda cluster, bottom is generic):
        /data/miniconda3/envs/spike_env/bin/time -v torchrun --standalone --nproc_per_node=2 DDP-bert_mlm.py 
        /usr/bin/time -v torchrun --standalone --nproc_per_node=2 DDP-bert_mlm.py 
"""
...
def main():
    ...
    results_dir = os.path.join(os.path.dirname(__file__), f'../../../results/run_results/DDP-bert_mlm')
    ...
    save_as = f"DDP-bert_mlm-RBD-train_{len(train_dataset)}_test_{len(test_dataset)}"
